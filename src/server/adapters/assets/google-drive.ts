...
export async function ingestGoogleDriveAsset(referenceInput: unknown, options: { accessToken: string; fetchImpl?: FetchLike; ingest?: typeof prepareAssetIntake }, execution: CommandExecution) {
  const fetched = await fetchGoogleDriveAsset(referenceInput, options);
  return (options.ingest ?? prepareAssetIntake)(fetched.descriptor, fetched.bytes, execution);
}
